/*
 * Zoom de texto da apresentação IPR 2.0 — Encontro de Chefias.
 *
...
(function () {
  ...
  /* Cada slide pode declarar `--tz-base` no seu proprio <style> para dizer em que
     tamanho ele foi fechado — um slide com pouco texto sobe a base para preencher
     os 1080px, um denso fica em 1. O zoom do usuario multiplica essa base, de modo
     que + / - continuam mexendo em todos os slides na mesma proporcao. */
  function baseOf() {
    var v = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--tz-base'));
    return (v && v > 0) ? v : 1;
  }

  function apply(showFeedback) {
    var root = document.documentElement;
    root.style.setProperty('--tz', String(zoom * baseOf()));
    if (zoom > 1) {
